...
AddEventHandler("rs_weaponsrack:Server:RequestRackItems", function(rackId, openMenu)
    ...
    local isOwner = Character and Character.identifier == rack.owner_identifier and Character.charIdentifier == rack.owner_charid

    TriggerClientEvent("rs_weaponsrack:Client:ReceiveRackItems", src, rackId, RacksInventory[invId], false)

    if isOwner and openMenu then
        TriggerClientEvent("rs_weaponsrack:Client:ReceiveRackItems", src, rackId, RacksInventory[invId], true)
    elseif not isOwner and openMenu then
...
AddEventHandler("rs_weaponsrack:Server:ReceiveCurrentWeapon", function(rackId, weaponData)
    ...
    local rack = loadedRacks[rackId]
    if not rack or rack.owner_identifier ~= Character.identifier or rack.owner_charid ~= Character.charIdentifier then
        Core.NotifyLeft(src, Config.Notify.Rack, Config.Notify.NotOwner, "menu_textures", "cross", 3000, "COLOR_RED")
        return
    end
    ...
    exports.vorp_inventory:subWeapon(src, weaponData.weaponId, function(success)
        if success then
            table.insert(RacksInventory[invId], {
                ...
                serial = weaponData.serial or "rack"
            })
