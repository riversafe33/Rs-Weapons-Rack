...
function OpenRackMenu(rackId, weapons)
    Menu.CloseAll()
    ...
    for slot, weapon in ipairs(weapons) do
        if type(weapon) == "table" and weapon.name and weapon.weaponId then
            table.insert(MenuElements, {
                ...
                value = weapon.weaponId, -- 🔥 Enviar solo el weaponId
    ...
    if #MenuElements == 0 then
        table.insert(MenuElements, {
            label = Config.Notify.Menu.Empty,
            value = "none",
            desc = Config.Notify.Menu.NotWeapons
        })
    end

    Menu.Open("default", GetCurrentResourceName(), "rack_menu_" .. rackId, {
        title = Config.Notify.Menu.Tittle,
    ...
    function(data, menu)
        if data.current.value ~= "none" then
            TriggerServerEvent("rs_weaponsrack:Server:TakeWeapon", rackId, data.current.value)
        end
        menu.close()
    end,
    function(data, menu)
...
AddEventHandler("rs_weaponsrack:Client:ReceiveRackItems", function(rackId, weapons, openMenu)
    ...
    for slot, weapon in ipairs(weapons) do
        if type(weapon) == "table" and weapon.name then
            local weaponData
            local useSecondRack = slot > 6 -- slots 7+ usan segundo lado
...
local openMenuPrompt = Uiprompt:new(`INPUT_DYNAMIC_SCENARIO`, "Open shelf", rackPromptGroup)
openMenuPrompt:setHoldMode(true)

local storeWeaponPrompt = Uiprompt:new(`INPUT_RELOAD`, "Put away weapon", rackPromptGroup)
storeWeaponPrompt:setHoldMode(true)

local pickUpRackPrompt = Uiprompt:new(`INPUT_OPEN_SATCHEL_MENU`, "Pick up rack", rackPromptGroup)
    ...
    for rackId, rack in pairs(loadedRacks) do
        ...
        if #(playerCoords - rackPos) < 1.5 then
            if prompt == openMenuPrompt then
                TriggerServerEvent("rs_weaponsrack:Server:RequestRackItems", rackId, true)
            elseif prompt == storeWeaponPrompt then
                TriggerEvent("rs_weaponsrack:Client:RequestCurrentWeapon", rackId)
